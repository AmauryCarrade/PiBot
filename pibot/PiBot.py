...
class PiBot(object):
	...
	def raw(self, raw):
		...
		event = Event()
		event.raw = raw
		call_event(Hook.RAW_MESSAGE_SENT, event, self)

		self._debug("--> " + event.raw + "\n")
		self._irc.send(_b(event.raw + '\r\n'))
	...
	def send_message(self, message, user = None):
		...
		if user == self.channel: call_event(Hook.CHANNEL_MESSAGE_SENT, event, self)
		else:                    call_event(Hook.PRIVATE_MESSAGE_SENT, event, self)

		call_event(Hook.MESSAGE_SENT, event, self)
		
		self.raw('PRIVMSG ' + event.user + ' :' + event.message)
	...
	def send_notice(self, message, user = None):
		...
		call_event(Hook.NOTICE_SENT, event, self)
		
		self.raw('NOTICE ' + event.user + ' :' + event.message)
	...
	def launch(self):
		...
		while self._retry:
			...
			self._irc.settimeout(15) # Used to force the loop to loop at least every 15 seconds.
			...
			while self._irc is not None:
				...
				try:
					...
					if time.time() - last_successful_recv > self.ping_timeout:

						self._fatal("Ping timeout! Nothing received from the server since " + str(self.ping_timeout) + " seconds.")
						self._info("Retrying in " + str(self.delay_before_reconnection) + " seconds...")

						self._retry = True
						time.sleep(self.delay_before_reconnection)
					continue
